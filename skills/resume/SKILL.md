...
4. **Resume the cycle at the exact point.** Whatever the handoff carried
   becomes `WORK.md` lines or immediate action. With the Compozy runtime
   active, reattach the handoff's session ids first (`compozy.md`,
   parallelism row) and carry them into `WORK.md`/the trail before
   treating any item as un-delegated; then delete `.batuta/handoff.md`
   — it is a transit note, consumed on arrival.
