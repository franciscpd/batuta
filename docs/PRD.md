...
1. **Brief** — orquestrador monta o task brief: objetivo, contexto, arquivos
   relevantes, convenções do perfil/template, critérios de aceite, escopo (lista fechada de caminhos alteráveis).
2. **Delegar** — invoca o adapter via Bash (`codex exec …`, `opencode run …`).
3. **Verificar** — checagem de escopo (`git diff --name-only` contra a lista do brief) + `git diff` review pelo orquestrador + testes do projeto +
...
| Verificação endurecida | Documento central `verification.md` na raiz (padrão dormante dos demais); relato do executor nunca é evidência — cada critério re-executado pela menor prova pública; scans descritivos de higiene de teste no diff (skip adicionado, assertion enfraquecida, mock escondendo dependência real, snapshot sem justificativa = falha; caminho feliz apenas = feedback); checklist de slop dentro do diff review | Sintoma de uso real: executor declara completo sem o diff sustentar (commits atômicos falhos); scans descritivos custam um grep e envelhecem devagar por serem agnósticos de stack; destilado do `agent-output-audit` e `deslop` do pedronauck/skills — spec de destilação em `docs/superpowers/specs/` |
| Brief endurecido | Duas seções novas no brief (Step 2): Expected evidence (o que o executor reporta — arquivos, comandos com saída real, incerteza declarada; é o que o Step 4 confere, nunca confia) e Stop conditions (quando parar e reportar em vez de improvisar); leis de teste condicionais no brief quando os critérios envolvem testes; sweep de "como" vazado nas lanes média+ (baratas mantêm prescrição de propósito); lacuna explícita `Unknown — <motivo>`; anti-workaround como item `Never:` do `templates/generic.md` com válvula `// WORKAROUND: <motivo>` julgada na verificação | Alimenta a verificação endurecida com material conferível (evidência prometida vs. diff) e corta os dois modos de falha do executor barato: improvisar diante do inesperado e calar sinal em vez de consertar fonte; destilado do delegation packet (`herdr-orchestration`), `to-prompt`, `no-workarounds` e `testing-boss` do pedronauck/skills |
